...
private:
        ...
        while(z->parent != nullptr && z->parent->color == RED){
            if(z->parent->parent != nullptr && z->parent == z->parent->parent->left){
                ...
                if(y != nullptr && y->color == RED ){
                    ...
                    if(z->parent != nullptr && z == z->parent->right){
                        z = z->parent;
                        left_rotate(z);
                    }
                    z->parent->color = BLACK;
                ...
                if(y != nullptr && y->color == RED){
                    ...
                    if(z->parent != nullptr && z == z->parent->left){
                        z = z->parent;
                        right_rotate(z);
                    }
                    z->parent->color = BLACK;
    ...
    size_t internal_node_count(Node<K, V> *node) const {
        // TODO
        //base case: if tree is empty, or its a leaf
        if (node == nullptr || (node->right==nullptr && node->left==nullptr)) {
            return 0;
        }
        //else add 1 for each non-leaf node
    ...
    int diameter(Node<K, V> *node) const {
        ...
        if(node == nullptr){
            ...
            result += 1 + diameter(node->right);
        }
        else if(node->right == nullptr && node->left != nullptr){
            result += 1 + diameter(node->left);
        }
        else{
            result += 2 + height(node->left) + height(node->right);
        }
        return result;
    }
